...
public final class KeycloakStack {
  ...
  public record Stack(
      String baseUrl, String realm, String migratorClientId, String migratorClientSecret) {}
  ...
  public static Stack bootstrap(String baseUrl, String realmName) {
    ...
    try {
      ...
      enableUnmanagedAttributePolicy(
          baseUrl, admin.tokenManager().getAccessTokenString(), realmName);
  ...
  private static void createOrUpdateRealm(Keycloak admin, String realmName) {
    boolean exists =
        admin.realms().findAll().stream().anyMatch(r -> realmName.equals(r.getRealm()));
    if (!exists) {
  ...
  /**
   * The /users/profile endpoint isn't exposed by the admin client at this version, so we use a
   * plain JAX-RS call with the admin's current token.
   */
  ...
  private static String createMigratorClient(Keycloak admin, String realm) {
    ...
    String realmMgmtClientUuid =
        admin.realm(realm).clients().findByClientId("realm-management").get(0).getId();
    RoleRepresentation realmAdmin =
        admin
            .realm(realm)
            .clients()
            .get(realmMgmtClientUuid)
            .roles()
            .get("realm-admin")
            .toRepresentation();
    admin
        ...
        .get(sa.getId())
        .roles()
        .clientLevel(realmMgmtClientUuid)
        .add(List.of(realmAdmin));
    return secret;
  }
}
