...
class SlowMigrationIT {
    ...
    void setUp() {
        ...
        kc = new PhaseTwoKeycloakContainer()
    ...
    void federationComponent_can_be_installed_with_upstream_provider_id() {
        ...
        var components = realmAdmin.realm(REALM).components().query(
                realmAdmin.realm(REALM).toRepresentation().getId(),
                "org.keycloak.storage.UserStorageProvider");
        assertThat(components)
                .anyMatch(c -> "workos-legacy-migration-it".equals(c.getName())
    ...
    void getKnownUser_returns_200_with_workos_shape() throws Exception {
        ...
        assertThat(body.path("attributes").path("workos.id").get(0).asText()).isEqualTo("user_TEST_alice");
    ...
    void postCorrectPassword_returns_200() throws Exception {
        workos.server().stubFor(WireMock.post(WireMock.urlPathEqualTo("/user_management/authenticate"))
    ...
    private HttpResponse<String> postPassword(String username, String password, String bearer) throws Exception {
        HttpRequest.Builder b = HttpRequest.newBuilder()
                .uri(URI.create(stack.baseUrl() + "/realms/" + REALM + "/workos-legacy/" + username))
    ...
    private void stubWorkos() {
        ...
        workos.server().stubFor(WireMock.get(WireMock.urlPathEqualTo("/user_management/users"))
        ...
        workos.server().stubFor(WireMock.get(WireMock.urlPathEqualTo("/user_management/users/user_TEST_alice/identities"))
