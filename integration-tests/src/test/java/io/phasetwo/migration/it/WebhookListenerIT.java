...
class WebhookListenerIT {
  ...
  @Container
  static final PhaseTwoKeycloakContainer kc =
  ...
  void validSignature_returns_200() throws Exception {
    String body =
        "{\"id\":\"evt_1\",\"event\":\"user.updated\",\"data\":{\"id\":\"user_TEST_alice\",\"email\":\"alice@acme.example.com\",\"first_name\":\"Alice\",\"last_name\":\"Updated\"},\"created_at\":\"2026-01-10T12:00:00Z\"}";
  ...
  void unknownPublicId_returns_404() throws Exception {
    ...
    HttpRequest req =
        HttpRequest.newBuilder()
            .uri(
                URI.create(
                    stack.baseUrl() + "/realms/" + REALM + "/workos-webhook/not-the-right-id"))
  ...
  void userDeleted_removes_user() throws Exception {
    String body =
        "{\"id\":\"evt_5\",\"event\":\"user.deleted\",\"data\":{\"id\":\"user_TEST_alice\"},\"created_at\":\"2026-01-10T12:00:00Z\"}";
    HttpResponse<String> resp = post(body, sign(body));
    assertThat(resp.statusCode()).isEqualTo(200);
  ...
  private void seedExistingUser() {
    ...
    try (var r = realmAdmin.realm(REALM).users().create(u)) {
      if (r.getStatus() != 201)
        throw new IllegalStateException("seed user failed: " + r.getStatus());
    }
  }
