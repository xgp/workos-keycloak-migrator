...
import io.phasetwo.migration.migrator.Main;
 ...
 * Asserts the happy path (CREATED counts) and then re-runs to assert idempotency (SKIPPED
 * reason=unchanged).
 */
@Testcontainers
...
class BulkMigratorIT {
  ...
  @Container static final PhaseTwoKeycloakContainer kc = new PhaseTwoKeycloakContainer();
  ...
  void happyPath_imports_workos_state() {
    try (LogCapture log = new LogCapture("io.phasetwo.migration.migrator.Steps")) {
  ...
  private int runMigrator() {
    String fingerprint = io.phasetwo.migration.common.util.Hashes.fingerprint("test-key");
  ...
  private void stubWorkos() {
    ...
    workos.stubJson(
        "/user_management/users/user_TEST_alice/identities", "users-identities-empty.json");
    workos.stubJson(
        "/user_management/users/user_TEST_bob/identities", "users-identities-empty.json");
    workos.stubJson(
        "/user_management/users/user_TEST_carol/identities", "users-identities-empty.json");
  }
