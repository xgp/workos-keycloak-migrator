...
import io.phasetwo.wkm.common.workos.model.WIdentity;
...
public class UserSync implements EntitySync<WUser> {
    ...
    public SyncResult sync(WUser u) {
        ...
        Optional<UserRepresentation> byEmail = existing.isPresent()
                ? existing
                : Lookups.userByEmail(ctx.realm(), u.email());
        ...
        Map<String, List<String>> attrs = target.getAttributes() == null ? new LinkedHashMap<>() : new LinkedHashMap<>(target.getAttributes());
        ...
        String hashSeed = JsonUtil.mapper().valueToTree(
                Map.of(
                        "u", u,
                        "src", ctx.source())).toString();
        ...
        // requiredActions
        List<String> required = target.getRequiredActions() == null ? new ArrayList<>() : new ArrayList<>(target.getRequiredActions());
        ...
        if (existing.isPresent()) {
            if (noChange) {
                return SyncResult.skipped(ENTITY, u.id(), "unchanged").withKeycloakId(existing.get().getId());
            }
    ...
    private static String mapProviderToAlias(String provider) {
        return "oauth-" + (provider == null ? "unknown" : provider.toLowerCase(Locale.ROOT).replace("oauth", ""));
    }
