package io.phasetwo.wkm.common.keycloak;
...
public final class Lookups {
    ...
    public static Optional<UserRepresentation> userByWorkosId(RealmResource realm, String workosId) {
        List<UserRepresentation> hits = realm.users()
                .searchByAttributes(AttributeKeys.WORKOS_ID + ":" + workosId);
        ...
        hits = realm.users()
                .searchByAttributes("q=" + AttributeKeys.WORKOS_ID + ":" + workosId);
        if (hits != null && !hits.isEmpty()) return Optional.of(fullUser(realm, hits.get(0).getId()));
    ...
    public static Optional<OrganizationRepresentation> orgByWorkosId(PhaseTwo pt, String realm, String workosId) {
        OrganizationsResource orgs = pt.organizations(realm);
        // PT search supports q=k:v attribute search
        List<OrganizationRepresentation> hits = orgs.get(
                Optional.empty(),
    ...
    public static Optional<OrganizationRepresentation> orgByName(PhaseTwo pt, String realm, String name) {
        List<OrganizationRepresentation> hits = pt.organizations(realm)
                .get(Optional.of(name), Optional.of(0), Optional.of(50));
        if (hits != null) {
            for (OrganizationRepresentation o : hits) {
    ...
    public static Optional<IdentityProviderRepresentation> idpByAlias(RealmResource realm, String alias) {
        try {
