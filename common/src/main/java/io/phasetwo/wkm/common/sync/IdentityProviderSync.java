...
public class IdentityProviderSync implements EntitySync<WConnection> {
    ...
    public SyncResult sync(WConnection c) {
        ...
        if ("saml".equals(mappedProviderId) && c.options() != null && c.options().signingCert() != null) {
        ...
        try {
            ...
            return SyncResult.partial(ENTITY, c.id(), alias, "incomplete_config")
                    .withKeycloakId(alias);
        } catch (jakarta.ws.rs.NotFoundException e) {
            String created = ctx.phaseTwo()
                    .organizations(ctx.realmName())
                    .organization(org.get().getId())
                    .identityProviders()
                    .create(rep);
    ...
    static String mapProviderId(String connectionType) {
        ...
        return switch (connectionType) {
            ...
            case "SlackOAuth", "DiscordOAuth", "BitbucketOAuth", "XeroOAuth",
                    "IntuitOAuth", "SalesforceOAuth", "VercelOAuth", "VercelMarketplaceOAuth" -> "oidc";
