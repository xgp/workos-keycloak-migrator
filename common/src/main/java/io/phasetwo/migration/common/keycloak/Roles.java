...
public final class Roles {
  ...
  /**
   * Ensure the {@value AttributeKeys#SCIM_MANAGED_ROLE} realm role exists. Idempotent — if a role
   * with the same name already exists we leave it alone.
   */
  public static void ensureScimManagedRole(RealmResource realm) {
    ...
    try {
      ...
      log.warnf(
          "could not create realm role %s: %s", AttributeKeys.SCIM_MANAGED_ROLE, e.toString());
    }
  }
