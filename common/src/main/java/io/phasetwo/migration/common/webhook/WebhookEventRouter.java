...
public class WebhookEventRouter {
    ...
    try {
      return switch (type) {
        ...
        case "organization_membership.created", "organization_membership.updated" ->
            Optional.of(
                new OrganizationMembershipSync(ctx).sync(convert(data, WOrgMembership.class)));
        ...
        case "dsync.user.created", "dsync.user.updated" -> Optional.of(syncDirectoryUser(data));
        ...
        case "organization_domain.created",
            ...
            // domain events are handled by re-syncing the parent org if WorkOS supplies
            // organization_id
  ...
  private SyncResult deleteOrg(JsonNode data) {
    ...
    return io.phasetwo.migration.common.keycloak.Lookups.orgByWorkosId(
            ctx.phaseTwo(), ctx.realmName(), id)
        .map(
            o -> {
  ...
  private SyncResult removeMembership(JsonNode data) {
    ...
    var org =
        io.phasetwo.migration.common.keycloak.Lookups.orgByWorkosId(
            ctx.phaseTwo(), ctx.realmName(), orgId);
    ...
    ctx.phaseTwo()
        ...
        .memberships()
        .remove(user.get().getId());
    return SyncResult.deleted(
        "organization_membership", null, org.get().getId() + ":" + user.get().getId());
  }
  ...
  private SyncResult syncDirectoryUser(JsonNode data) {
    ...
    io.phasetwo.migration.common.workos.model.WDirectoryUser du =
        convert(data, io.phasetwo.migration.common.workos.model.WDirectoryUser.class);
  ...
  private SyncResult syncOrgRole(JsonNode data) {
    ...
    String orgId = textOr(data, "organization_id");
    if (orgId == null && role != null && role.resourceTypeSlug() != null)
      orgId = textOr(data, "resource_id");
    if (orgId == null)
      return SyncResult.failed(
          "organization_role", role == null ? null : role.id(), "missing org id");
