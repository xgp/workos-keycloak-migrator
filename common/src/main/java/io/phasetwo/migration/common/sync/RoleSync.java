...
public class RoleSync implements EntitySync<WRole> {
  ...
  public SyncResult syncOrganizationRole(WRole r, String workosOrgId) {
    ...
    Optional<OrganizationRepresentation> org =
        Lookups.orgByWorkosId(ctx.phaseTwo(), ctx.realmName(), workosOrgId);
    if (org.isEmpty()) {
      return SyncResult.skipped("organization_role", r.id(), "org_not_yet_migrated:" + workosOrgId);
    }
    OrganizationRolesResource roles =
        ctx.phaseTwo().organizations(ctx.realmName()).organization(org.get().getId()).roles();

    String permString =
        r.permissions() == null || r.permissions().isEmpty()
            ? ""
            : "[permissions:" + String.join(",", r.permissions()) + "] ";
    String description =
        (permString + (r.description() == null ? "" : r.description()) + " [wos:" + r.id() + "]")
            .trim();
    ...
    OrganizationRoleRepresentation rep =
        existing != null ? existing : new OrganizationRoleRepresentation();
    rep.setName(r.slug());
