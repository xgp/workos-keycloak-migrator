...
/**
 * Materialises WorkOS Directory Groups as Keycloak groups under a per-organization parent {@code
 * /org-{pt_org_id}} — the parent name embeds the Phase Two organization id (not the WorkOS id) so
 * it stays stable for admins navigating the KC group tree alongside the PT org UI.
 */
@JBossLog
public class DirectoryGroupSync implements EntitySync<WDirectoryGroup> {
  ...
  public SyncResult sync(WDirectoryGroup g) {
    ...
    List<GroupRepresentation> children =
        parent.getSubGroups() == null ? List.of() : parent.getSubGroups();
