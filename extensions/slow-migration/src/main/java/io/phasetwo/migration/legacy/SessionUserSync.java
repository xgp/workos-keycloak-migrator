...
/**
 * Performs an in-process full-sync of a single user using only Keycloak SPIs + Phase Two's {@link
 * OrganizationProvider}. Mirrors what {@code UserSync}, {@code OrganizationMembershipSync} and
 * {@code DirectoryUserSync} in {@code common} do in the bulk migrator — minus the bits that require
 * the Keycloak/Phase Two admin clients (organisations are touched via the in-process provider
 * instead).
 */
@JBossLog
final class SessionUserSync {
  ...
  /**
   * Enrich the local user identified by {@code email}. Returns {@code true} when the user was found
   * and processed; {@code false} when no local user exists (the slow-migration import didn't
   * materialise it, so there is nothing to do).
   */
  ...
  private void applyAttributes(UserModel user, WUser wu) {
    ...
    if (wu.metadata() != null) {
      wu.metadata()
          .forEach((k, v) -> user.setSingleAttribute(AttributeKeys.WORKOS_METADATA_PREFIX + k, v));
    }
  ...
  private void applyOrgMemberships(UserModel user, WUser wu) {
    ...
    for (WOrgMembership m : userMemberships) {
      ...
      if (org == null) {
        log.debugf(
            "PT org for WorkOS %s not migrated yet; skipping membership", m.organizationId());
        continue;
      }
      if (!org.hasMembership(user)) {
        ...
        log.infof(
            "post-commit sync: granted %s membership in %s", user.getUsername(), org.getName());
      }
  ...
  private void applyScimManaged(UserModel user, String email) {
    ...
    if (d.directoryId() != null)
      user.setSingleAttribute(AttributeKeys.SCIM_DIRECTORY_ID_ATTR, d.directoryId());
