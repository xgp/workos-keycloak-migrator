...
/**
 * A no-op-during-the-main-txn {@link KeycloakTransaction} that, on {@code commit()}, opens a fresh
 * Keycloak session and runs {@link SessionUserSync#enrich} for the supplied email. Enlisted by
 * {@link WorkOSLegacyResource#getUser} via {@code
 ...
 * the database and we can grant roles, federated identities, and Phase Two organisation memberships
 * safely.
 */
@JBossLog
final class EnrichUserTransaction implements KeycloakTransaction {
  ...
  EnrichUserTransaction(
      KeycloakSession session, String realmName, String email, String apiBaseUrl, String apiKey) {
