...
public class WorkOSWebhookProviderFactory implements RealmResourceProviderFactory {
  ...
  static final List<String> SUBSCRIBED_EVENTS =
      List.of(
          "user.created",
          "user.updated",
          "user.deleted",
   ...
   * Otherwise a comma-separated allowlist. Set via the {@code realms} provider config (env var
   * {@code KC_SPI_REALM_RESTAPI_EXTENSION_WORKOS_WEBHOOK_REALMS}).
   */
  ...
  public void init(Config.Scope config) {
    ...
    this.eventToleranceMillis =
        Long.parseLong(configOrDefault(config, "eventTolerance", "300")) * 1000L;
    ...
    this.provisionWebhook =
        Boolean.parseBoolean(configOrDefault(config, "provisionWebhook", "true"));
  }
  ...
  private void provisionRealm(RealmModel realm) {
    ...
    if (base == null || base.isBlank()) {
      log.warnf(
          "realm %s: no public base URL — set spi-realm-restapi-extension-workos-webhook-webhook-base-url or realm attribute frontendUrl",
          realm.getName());
      return;
    }
    ...
    if (!endpointUrl.startsWith("https://")) {
      log.warnf(
          "realm %s: WorkOS requires an HTTPS endpoint; skipping provisioning (computed url: %s). "
    ...
    for (WWebhookEndpoint ep : existing) {
      if (endpointUrl.equals(ep.endpointUrl())) {
        ...
        if (ep.secret() != null)
          realm.setAttribute(AttributeKeys.REALM_WEBHOOK_SECRET, ep.secret());
    ...
    if (created.secret() != null)
      realm.setAttribute(AttributeKeys.REALM_WEBHOOK_SECRET, created.secret());
    log.infof(
        "realm %s: created WorkOS webhook %s at %s", realm.getName(), created.id(), endpointUrl);
  }
