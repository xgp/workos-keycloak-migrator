 ...
 * signature, and applies in-process mutations where it can. For events that require the full Phase
 * Two / Keycloak admin-client surface (organization create, idp upsert, SCIM stubs), the resource
 * marks the realm as needing reconciliation and the next bulk migrator run picks them up via the
 * cursor + state mechanism in {@code common}. This trade-off avoids dragging the Keycloak admin
 * client into the Keycloak server's own classpath.
 */
@JBossLog
public class WorkOSWebhookResource {
  ...
  private void dispatch(RealmModel realm, WebhookEvent event) {
    ...
    switch (type) {
      ...
      default ->
          log.infof(
              "realm %s: webhook %s accepted — marked for bulk resync", realm.getName(), type);
    }
  ...
  private void handleUserDeleted(RealmModel realm, JsonNode data) {
    ...
    if (u != null) {
      ...
      log.infof(
          "realm %s: removed user %s via webhook (workos id %s)",
          realm.getName(), u.getUsername(), workosId);
    }
  }
  ...
  private void handleUserUpsert(RealmModel realm, JsonNode data) {
    ...
    if (u == null) {
      ...
      log.infof(
          "realm %s: webhook user.created/updated for %s deferred to next bulk run",
          realm.getName(), email);
      return;
    }
    ...
    log.infof(
        "realm %s: updated user %s via webhook (workos id %s)", realm.getName(), email, workosId);
  }
  ...
  private void handleMembershipDeleted(RealmModel realm, JsonNode data) {
    ...
    log.infof(
        "realm %s: organization_membership.deleted deferred to next bulk run (%s)",
